...
async function listTeachers(req, res) {
  ...
  const totalRes = await pool.query(
    `SELECT COUNT(*)::int AS total FROM teachers ${where}`,
    params,
  );
...
async function getTeacher(req, res) {
  ...
  const { rows: schedules } = await pool.query(
    `SELECT s.id, s.date, s.start_time, s.end_time, s.capacity, s.price, s.status,
            COUNT(a.id)::int AS booked
     FROM schedules s
     LEFT JOIN appointments a ON a.schedule_id = s.id AND a.status = 'confirmed'
     WHERE s.teacher_id = $1
     GROUP BY s.id, s.date, s.start_time, s.end_time, s.capacity, s.price, s.status
